...
> 历史计划：2026-09-16 起小程序所有版本仅连接云托管，下文早期本地/LAN 联调内容不再作为操作指引。当前配置与验收以 [小程序说明](../miniprogram/README.md) 为准，Web/Python 本地调试仍保留。
...
- [x] Audio Check 音量检测
- [x] 10 分钟自动续录机制
- [ ] iPhone PCM 真机验证（等待真机测试）
- [ ] Android PCM 真机验证（等待真机测试）
...
- 真机测试：测试 AppID 已配置，需要通过“预览”二维码在手机微信中验证实际麦克风 PCM。
- 正式发布：需要备案域名、HTTPS/WSS 和云服务器。
- iOS/Android 验收：需要各一台测试设备。

## 2026-08-30 开发者工具验收

- 项目编译错误为 0；Meeting Setup → Audio Check → Live → Meeting Result 全流程通过。灰度基础库偶发的 `DOMNodeRemoved` 是开发者工具自身的渲染层提示，不影响小程序运行。
- Pause 后计时器与收音暂停，Resume 后恢复同一会议，Stop 后成功保存 Markdown 会议稿。
- 开发者工具不稳定触发录音启动事件时，会显示 `Simulator Check` 并完成交互验收；真机仍使用真实麦克风录音。
- 通过本地 WebSocket 服务分别发送中英文测试音频，验证了自动语种识别、英文译中文和中文译英文。
